...
function addPath(count) {
	...
	for (i = 0; i < count; i += 1) {
                ...
                z[i] = Math.floor(Math.random() * 200);
		vert = new Vert(x[i], y[i], z[i]);
		vert.mesh.obj = vert;
...
function positionPathCamera() {
	...
	pos = geometry.path.getPointAt(t);
        ...
        for(i = 0; i < x.length; i++){
                if(pos.x <= x[i]+1 && pos.x >= x[i]-1 && pos.y <=y[i]+1 && pos.y >= y[i]-1 && pos.z <= z[i]+1 && pos.z >= z[i]-1){
                    if(ANIMATE !== false){
                        lastTime = Date.now();
                        looptime = 0;
                        ANIMATE = false;
...
var Vert = function (x, y, z) {
	...
	sphere = new THREE.SphereGeometry(10, 10, 10);
        ...
        texture = THREE.ImageUtils.loadTexture('Stone.jpg', {}, function() {
